<x-app-layout>
    ...
    <x-card>
        <form method="POST" action="{{ route('servicios.update', $servicio) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="descripcion" name="descripcion" rows="4"
                    class="w-full rounded-lg border-2 border-gray-300 focus:border-black focus:ring-black">{{ old('descripcion', $servicio->descripcion) }}</textarea>
                @error('descripcion')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" id="duracion" name="duracion" value="{{ old('duracion', $servicio->duracion) }}" required min="1"
                        class="w-full rounded-lg border-2 border-gray-300 focus:border-black focus:ring-black">
                    @error('duracion')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="precio" name="precio" value="{{ old('precio', $servicio->precio) }}" required min="0" step="0.01"
                        class="w-full rounded-lg border-2 border-gray-300 focus:border-black focus:ring-black">
                    @error('precio')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
